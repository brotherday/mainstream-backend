...
// FileMetaData contains all meta data concerning the File contract.
var FileMetaData = &bind.MetaData{
...
// Deprecated: Use FileMetaData.ABI instead.
var FileABI = FileMetaData.ABI
...
// FileInitializedIterator is returned from FilterInitialized and is used to iterate over the raw logs and unpacked data for Initialized events raised by the File contract.
type FileInitializedIterator struct {
	Event *FileInitialized // Event containing the contract specifics and raw log
...
func (it *FileInitializedIterator) Next() bool {
...
func (it *FileInitializedIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *FileInitializedIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// FileInitialized represents a Initialized event raised by the File contract.
type FileInitialized struct {
	Version uint8
...
func (_File *FileFilterer) FilterInitialized(opts *bind.FilterOpts) (*FileInitializedIterator, error) {

	logs, sub, err := _File.contract.FilterLogs(opts, "Initialized")
	if err != nil {
		return nil, err
	}
	return &FileInitializedIterator{contract: _File.contract, event: "Initialized", logs: logs, sub: sub}, nil
}

// WatchInitialized is a free log subscription operation binding the contract event 0x7f26b83ff96e1f2b6a682f133852f6798a09c465da95921460cefb3847402498.
//
// Solidity: event Initialized(uint8 version)
func (_File *FileFilterer) WatchInitialized(opts *bind.WatchOpts, sink chan<- *FileInitialized) (event.Subscription, error) {

	logs, sub, err := _File.contract.WatchLogs(opts, "Initialized")
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				...
				event := new(FileInitialized)
				if err := _File.contract.UnpackLog(event, "Initialized", log); err != nil {
					return err
				}
				event.Raw = log
...
//
// Solidity: event Initialized(uint8 version)
func (_File *FileFilterer) ParseInitialized(log types.Log) (*FileInitialized, error) {
	event := new(FileInitialized)
	if err := _File.contract.UnpackLog(event, "Initialized", log); err != nil {
		return nil, err
	}
